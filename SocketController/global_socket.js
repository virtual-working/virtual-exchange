  ...
  try {
    wss.on("connection", async (ws) => {
      if (ws.readyState === ws.OPEN) {
        ws.send(
          JSON.stringify({
            msg1: "WELCOME TO VIRTUAL EXCHANGE",
          })
        );
...
async function GetMarketTradeData(ws, symbol, market_type) {
  try {
    let items = await MarketTradeModel.findOne({
      symbol: symbol,
      market_type: market_type,
    }).select("symbol price amount isMaker");
    ws.send(
      JSON.stringify({ type: market_type + "_market_trade", content: items })
    );
    MarketTradeModel.watch([
      ...
      if (items != null)
        ws.send(
          JSON.stringify({
            type: market_type + "_market_trade",
            content: items,
          })
        );
    });
...
async function GetFutureMarketPrices(ws) {
  try {
    let items = await FutureQuoteModel.find({ market_type: "future" }).select(
      "symbol ask bid change changeDiff"
    );
...
async function GetFutureMarketTradeData(ws, symbol, market_type) {
  try {
    let items = await FutureMarketTradeModel.findOne({
      symbol: symbol,
      market_type: "future",
    }).select("symbol price amount isMaker");
    ...
    FutureMarketTradeModel.watch([
      ...
      if (items != null)
        ws.send(
          JSON.stringify({ type: "future_market_trade", content: items })
        );
    });
  } catch (err) {}
}
...
async function GetFutureOrderBooks(ws, pair) {
  try {
    if (pair != null || pair != "") {
      ...
      let book = await FutureOrderBookModel.findOne(
      ...
      FutureOrderBookModel.watch([
        ...
        if (book != null)
          ws.send(
            JSON.stringify(
              { type: "future_order_book", content: book },
              { _id: 0, __v: 0, created_at: 0 }
            )
          );
      });
...
async function GetFutureMarketInfo(ws, pair) {
  try {
    if (pair != null || pair != "") {
      let symbol = pair.replace("/", "").replace("_", "");
      let quote = await FutureQuoteModel.findOne(
        { symbol: symbol, market_type: "future" },
        { _id: 0, __v: 0, created_at: 0 }
      );
      ...
      FutureQuoteModel.watch([
        ...
        if (quote != null)
          ws.send(
            JSON.stringify({ type: "future_market_info", content: quote })
          );
      });
    }
  } catch (err) {}
...
async function GetSpotMarketPrices(ws) {
  try {
    let items = await SpotQuoteModel.find({ market_type: "spot" }).select(
      "symbol ask bid change changeDiff"
    );
...
async function GetSpoMarketTradeData(ws, symbol, market_type) {
  try {
    let items = await SpotMarketTradeModel.findOne({
      symbol: symbol,
      market_type: "spot",
    }).select("symbol price amount isMaker");
...
async function GetSpotOrderBooks(ws, pair) {
  try {
    if (pair != null || pair != "") {
      let symbol = pair.replace("/", "").replace("_", "");
      let book = await SpotOrderBookModel.findOne(
...
async function GetSpotMarketInfo(ws, pair) {
  try {
    if (pair != null || pair != "") {
      let symbol = pair.replace("/", "").replace("_", "");
      let quote = await SpotQuoteModel.findOne(
        { symbol: symbol, market_type: "spot" },
        { _id: 0, __v: 0, created_at: 0 }
      );
