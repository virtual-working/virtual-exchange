      ...
      if (verification == null) {
        ...
        let params4 = {
          params: {
            Bucket: "oxhain",
            Key:
              "Recidency/recidency1-" +
              timestamp +
              user_id +
              "." +
              req.body.recidencyFileExtension1,
            Body: file1New,
        ...
        if (req.body.recidencyFileExtension2 != null) {
          ...
          let params4 = {
            params: {
              Bucket: "oxhain",
              Key:
                "Recidency/recidency2-" +
                timestamp +
                user_id +
                "." +
                req.body.recidencyFileExtension2,
              Body: file2New,
              ContentType: "image/jpg",
            },
          };
        ...
        let verification = new RecidencyModel({
          ...
          status: 0,
          url:
            "https://oxhain.s3.us-east-2.amazonaws.com/Recidency/recidency1-" +
            timestamp +
            user_id +
            "." +
            req.body.recidencyFileExtension1,
          url2:
            "https://oxhain.s3.us-east-2.amazonaws.com/Recidency/recidency2-" +
            timestamp +
            user_id +
            "." +
            req.body.recidencyFileExtension2,
        ...
        verification.save(function (err) {
          if (err) {
            return res.json({
              status: "fail",
              message: "error",
              showableMessage: "Error while uploading Recidency",
            });
          } else {
            return res.json({
              status: "success",
              message: "success",
              showableMessage: "Recidency Uploaded",
            });
          }
        });
      } else {
        if (verification.status == 2) {
          ...
          RecidencyModel.findOneAndUpdate(
            ...
            if (err) {
              ...
              return res.json({
                status: "success",
                message: "success",
                showableMessage: "Recidency Updated",
              });
            }
          });
        } else {
          return res.json({
            ...
            showableMessage: "Recidency Already Uploaded",
          });
        }
      }
    } else {
