    ...
    if (user != null) {
      ...
      if (verification == null) {
        ...
        let verification = new VerificationModel({
          ...
          status: 0,
          url:
            "https://oxhain.s3.us-east-2.amazonaws.com/KYC/front-" +
            timestamp +
            user_id +
            "." +
            file1extension,
          url2:
            "https://oxhain.s3.us-east-2.amazonaws.com/KYC/back-" +
            timestamp +
            user_id +
            "." +
            file2extension,
          url3:
            "https://oxhain.s3.us-east-2.amazonaws.com/KYC/selfie-" +
            timestamp +
            user_id +
            "." +
            file3extension,
        ...
        verification.save(function (err) {
          if (err) {
            return res.json({
              status: "fail",
              message: "error",
              showableMessage: "Error while uploading KYC",
            });
          } else {
            return res.json({
              status: "success",
              message: "success",
              showableMessage: "KYC Uploaded",
            });
          }
        });
      } else {
        if (verification.status == 2) {
          ...
          return res.json({
            ...
            showableMessage: "KYC Already Uploaded",
          });
        }
      }
    } else {
