...
const withdrawOther = async (req, res) => {
  ...
  commission = commission * result; ////commission set for now
  let checkBal =
...
const withdrawNative = async (req, res) => {
  ...
  console.log(
    ...
    parseFloat(amounts) > 999,
    parseFloat(amounts)
  );

  if (parseFloat(amounts) > 999) {
    let result = parseInt(amounts / 1000);
  ...
  let fromWallet = await Wallet.findOne({
    address: getUserWallet.wallet_address,
    coin_id: coin_id,
    coinName: "BNB",
  }).exec();
  if (fromWallet == null)
  ...
  // let getNativeBalance = await getBalance(fromAddress);
  // console.log("getNativeBalance", getNativeBalance);
};
